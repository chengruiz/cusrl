...
class Mlp(Module):
    ...
    This class builds a sequential neural network model consisting of linear layers,
    activation functions, and optional dropout layers. The architecture can be
    flexibly defined by specifying the dimensions of the input, hidden, and output
    layers.
    ...
    Args:
        ...
        output_dim (int | None, optional):
            The dimension of the output. If `None`, the last element of `hidden_dims`
            is used as the output dimension, and the preceding elements define the
            hidden layers. Defaults to `None`.
        ...
        ends_with_activation (bool, optional):
            If `True`, an activation function is applied to the final output layer.
            Defaults to `False`.
