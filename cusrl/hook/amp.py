...
class AdversarialMotionPrior(Hook[ActorCritic]):
    ...
    Described in: "AMP: adversarial motion priors for stylized physics-based character control",
    https://dl.acm.org/doi/10.1145/3450626.3459670

    This hook modifies the agent's reward and provides a loss objective to train a
    discriminator network. The discriminator is trained to distinguish between
    transitions generated by the agent and transitions from an expert dataset. The
    agent is then rewarded for generating transitions that the discriminator
    ...
    Args:
        discriminator_factory (ModuleFactoryLike):
            A factory or class for creating the discriminator model. It will be called
            with input and output dimensions. The discriminator may not be recurrent.
        dataset_source (str | Array | Callable[[], Array]):
            The source of the expert motion dataset. Can be a file path to a `.npy` or
            `.pt` file, a `numpy.ndarray` / `torch.Tensor`, or a callable that returns
            a `numpy.ndarray` / `torch.Tensor`.
        state_indices (Slice | None, optional):
            A slice object to extract the relevant parts of the state for AMP. If `None`,
            it's assumed that the environment provides an "amp_obs" key in the transition
        ...
        loss_weight (float, optional):
            A weight for the total discriminator loss in the optimization objective.
            Defaults to 1.0.
        grad_penalty_weight (float, optional):
            The weight for the gradient penalty term in the discriminator loss. Defaults to 5.0.
    """
