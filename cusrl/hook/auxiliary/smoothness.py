    ...
    def objective(self, metadata, batch):
        ...
        if self._weight1_tensor is not None:
            ...
            smoothness_1st_order = (
                ...
                nn.functional.conv1d(action_sequence, self.conv_1st_order)  # ( N * ..., 1, L - 1 )
                .unflatten(0, padded_action.shape[1:])                      # ( N, ..., 1, L - 1 )
                .squeeze(-2)                                                # ( N, ..., L - 1 )
                .movedim(-1, 0)                                             # ( L - 1, N, ... )
            )
            # fmt: on
        ...
        if self._weight2_tensor is not None:
            ...
            smoothness_2nd_order = (
                nn.functional.conv1d(action_sequence, self.conv_2nd_order)  # ( N * ..., 1, L - 2 )
                .unflatten(0, padded_action.shape[1:])                      # ( N, ..., 1, L - 2 )
                .squeeze(-2)                                                # ( N, ..., L - 2 )
                .movedim(-1, 0)                                             # ( L - 2, N, ... )
            )
            # fmt: on
